...
async function mergeCollections() {
    try {
        ...
        const mergedData = await Toy.aggregate([
            ...
            {
                // Định dạng lại các field để khớp với Toy Schema
                $project: {
        ...
        // 4. Xóa dữ liệu cũ ở bảng Toy (nếu có) và chèn dữ liệu mới
        await Toy.deleteMany({});
        await Toy.insertMany(mergedData);

        console.log(`--- Thành công! Đã hợp nhất ${mergedData.length} bản ghi vào bảng Toy ---`);
