...
export default function ManageInspections() {
  ...
  const filteredInspections = inspections.filter(ins => {
    ...
    const matchesSearch = toyTitle.includes(search) || renterName.includes(search);
    const matchesType = typeFilter ? ins.type === typeFilter : true;
    const matchesCondition = conditionFilter ? ins.condition === conditionFilter : true;
        ...
        <Card className="border-0 shadow-sm rounded-4 overflow-hidden">
          <Card.Header className="bg-white py-4 border-0">
            <Row className="align-items-center">
              <Col md={4}>
                <InputGroup className="bg-light border-0 rounded-4 px-3 py-1">
                  ...
                  <Form.Control 
                    ...
                    placeholder="Tìm đồ chơi, khách hàng..." 
                    value={searchText}
              ...
              <Col md={3}>
                <Form.Select
                  className="bg-light border-0 rounded-4 py-2 text-muted shadow-none"
                  value={typeFilter}
                  onChange={(e) => setTypeFilter(e.target.value)}
                >
                  <option value="">Tất cả phân loại</option>
                  <option value="pickup">Giao đồ</option>
                  <option value="return">Nhận đồ</option>
                </Form.Select>
              </Col>
              <Col md={3}>
                <Form.Select
              ...
              <Col md={2} className="text-end text-muted small">
                 <div className="fw-medium">Tổng số: <span className="text-success fw-bold">{filteredInspections.length}</span></div>
              </Col>
            </Row>
          </Card.Header>
